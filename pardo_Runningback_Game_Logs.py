...
# PTransform: combine year and game_date fields and format to correct date format, replace '--' values with null and convert to appropriate type
class formatDate(beam.DoFn):
    def process(self, element):
        ...
        new_game_date = str(year) + "-" + game_date
        #new_game_date = datetime.strptime(new_game_date, '%Y-%m-%d')
        
        # ***CHANGE GAMEDATE TYPE BACK TO DATE***
...
with beam.Pipeline('DirectRunner', options=opts) as p:
    ...
    output_pcoll = query_results | 'Add year and format date' >> beam.ParDo(formatDate())
    
    # write PCollection to log file
    output_pcoll | 'Write to log 2' >> WriteToText('output.txt')
    
    qualified_table_name = PROJECT_ID + ':nfl_stats_modeled.Runningback_Game_Logs'
    table_schema = 'player_id:STRING,year:INTEGER,season:STRING,week:INTEGER,game_date:STRING,home_or_away:STRING,opponent:STRING,outcome:STRING,score:STRING,games_started:INTEGER,rushing_attempts:INTEGER,rushing_yards:INTEGER,yards_per_carry:FLOAT,longest_run:INTEGER,rushing_tds:INTEGER,receptions:INTEGER,receiving_yards:INTEGER,yards_per_reception:FLOAT,longest_reception:INTEGER,receiving_tds:INTEGER,fumbles:INTEGER,fumbles_lost:INTEGER'
